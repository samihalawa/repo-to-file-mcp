...
const ConvertRepositorySchema = z.object({
  repository: z.string().describe('Repository URL or GitHub shorthand (e.g., user/repo)'),
  compress: z
    .boolean()
    .default(true)
  ...
  includePatterns: z
    .string()
    .optional()
    .describe('Comma-separated glob patterns to include (e.g., "src/**/*.ts,**/*.md")'),
  ignorePatterns: z
    .string()
    .optional()
    .describe('Comma-separated glob patterns to ignore (e.g., "node_modules,dist")'),
});
    ...
    tools: [
      {
        ...
        description: 'Convert a remote GitHub repository into a single file for analysis',
        parameters: ConvertRepositorySchema,
      },
      {
        name: 'convert_local',
        description: 'Convert a local directory into a single file for analysis',
...
server.setRequestHandler(
  ...
  async (request) => {
    try {
      const { repository, compress, includePatterns, ignorePatterns } = request.parameters;
      ...
      // Build the command
      let command = `npx repomix --remote ${repository} --output ${outputPath}`;
      ...
      try {
        ...
        throw new McpError(`Failed to generate output file: ${stderr}`);
      }
...
server.setRequestHandler(
  ...
  async (request) => {
    try {
      const { directory, compress, includePatterns, ignorePatterns } = request.parameters;
      
      // Verify the directory exists
      try {
        const stats = await fs.stat(directory);
        if (!stats.isDirectory()) {
          throw new McpError(`Not a directory: ${directory}`);
        }
      } catch (error) {
        throw new McpError(`Directory does not exist: ${directory}`);
      }
  ...
  async (request) => {
    try {
      ...
      try {
        ...
        if (!stats.isFile()) {
          throw new McpError(`Not a file: ${filePath}`);
        }
      } catch (error) {
        throw new McpError(`File does not exist: ${filePath}`);
      }
